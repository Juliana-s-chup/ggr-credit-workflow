...
def canevas_create_or_edit(request, dossier_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, f"✓ Canevas de proposition {'créé' if is_new else 'mis à jour'} avec succès.")
            
            # Si nouveau canevas, rediriger vers upload documents
            if is_new:
                messages.info(request, "Canevas enregistré ! Veuillez maintenant uploader les documents requis.")
