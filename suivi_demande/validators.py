...
def validate_file_upload(uploaded_file: UploadedFile) -> Tuple[bool, str]:
    ...
    if uploaded_file.size > MAX_FILE_SIZE:
        return False, f"Fichier trop volumineux ({uploaded_file.size / 1024 / 1024:.1f} MB). Maximum: 10 MB"
    
    # Vérifier l'extension
    file_ext = os.path.splitext(uploaded_file.name)[1].lower()
    if file_ext not in ALLOWED_EXTENSIONS:
        return False, f"Extension '{file_ext}' non autorisée. Extensions acceptées: {', '.join(ALLOWED_EXTENSIONS)}"
    ...
    try:
        ...
        if mime_type not in ALLOWED_MIME_TYPES:
            return False, f"Type de fichier '{mime_type}' non autorisé. Types acceptés: PDF, JPG, PNG, DOC, DOCX"
