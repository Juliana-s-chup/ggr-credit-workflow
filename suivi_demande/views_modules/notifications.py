...
def notifications_list(request):
    ...
    notifications_list = Notification.objects.filter(utilisateur_cible=request.user).order_by(
        "-created_at"
    )
    ...
    return render(request, "suivi_demande/notifications.html", {"notifications": notifications})
...
def notifications_mark_all_read(request):
    ...
    if request.method == "POST":
        count = Notification.objects.filter(utilisateur_cible=request.user, lu=False).update(
            lu=True
        )

        if count > 0:
            messages.success(request, f"{count} notification(s) marquee(s) comme lue(s).")
        else:
...
def notifications_mark_read(request, pk: int):
    ...
    if request.method == "POST":
        ...
        # Rediriger vers la page precedente si fournie
        namespace = get_current_namespace(request)
