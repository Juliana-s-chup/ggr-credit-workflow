"""
Modèles pour l'application suivi_demande.
Gère les dossiers de crédit, les utilisateurs, les notifications et le workflow.
"""
...
class DossierCredit(models.Model):
    ...
    statut_agent = models.CharField(max_length=40, choices=DossierStatutAgent.choices, default=DossierStatutAgent.NOUVEAU, db_index=True)
    ...
    archived_by = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, blank=True, related_name="dossiers_archives")
    date_soumission = models.DateTimeField(default=timezone.now, db_index=True)
    date_maj = models.DateTimeField(auto_now=True)
    # Suivi du wizard (étapes)
    wizard_current_step = models.PositiveSmallIntegerField(default=1, choices=[
        (1, "Étape 1"),
        (2, "Étape 2"),
        (3, "Étape 3"),
        (4, "Étape 4"),
...
class PieceJointe(models.Model):
    TYPE_PIECE_CHOICES = [
        ...
        ("ATTESTATION_DOMICILIATION", "Attestation de domiciliation irrévocable"),
        ("ASSURANCE_DECES_INVALIDITE", "Assurance décès-invalidité"),
        ("AUTRE", "Autre"),
    ]
...
class CanevasProposition(models.Model):
    """Canevas de proposition de crédit NOKI NOKI."""
    ...
    statut_emploi = models.CharField(max_length=50, default="PRIVE", choices=[
        ("PRIVE", "Privé"),
        ("PUBLIC", "Public"),
    ])
    anciennete_emploi = models.CharField(max_length=100)  # Ex: "16 ans et 06 mois"
    type_contrat = models.CharField(max_length=20, default="CDI", choices=[
        ("CDI", "CDI"),
        ("CDD", "CDD"),
        ("STAGE", "Stage"),
        ("AUTRE", "Autre"),
    ...
    # Situation familiale
    situation_famille = models.CharField(max_length=20, default="MARIE", choices=[
    ...
    regime_matrimonial = models.CharField(max_length=100, blank=True, choices=[
    ...
    statut_logement = models.CharField(max_length=100, blank=True, choices=[
        ("LOCATAIRE", "Locataire"),
        ("PROPRIETAIRE", "Propriétaire"),
        ("AUTRES", "Autres (à préciser)"),
    ])
    numero_tf = models.CharField(max_length=100, blank=True)  # Si propriétaire
    logement_autres_precision = models.CharField(max_length=200, blank=True)
    
    # Nature du prêt en cours
    nature_pret_cours = models.CharField(max_length=20, default="NOKI", choices=[
        ("NOKI", "NOKI"),
        ("AUTRE", "Autre"),
    ])
    ...
    salaire_net_avant_endettement_fcfa = models.DecimalField(max_digits=12, decimal_places=2, default=0)
    capacite_endettement_brute_fcfa = models.DecimalField(max_digits=12, decimal_places=2, default=0)
    capacite_endettement_nette_fcfa = models.DecimalField(max_digits=12, decimal_places=2, default=0)
    
    # === SECTION 3: DÉTAILS DU CRÉDIT ===
    nature_pret = models.CharField(max_length=20, default="PRET", choices=[
        ("CAUTION", "Caution"),
        ("PRET", "Prêt"),
        ("DECOUVERT", "Découvert"),
    ])
    ...
    demande_periodicite = models.CharField(max_length=1, default="M", choices=[
        ("M", "Mensuelle"),
        ("T", "Trimestrielle"),
        ("S", "Semestrielle"),
        ("A", "Annuelle"),
    ...
    # Proposition
    proposition_montant_fcfa = models.DecimalField(max_digits=12, decimal_places=2, null=True, blank=True)
    proposition_duree_mois = models.IntegerField(null=True, blank=True)
    proposition_taux_pourcent = models.DecimalField(max_digits=5, decimal_places=2, null=True, blank=True)
    proposition_periodicite = models.CharField(max_length=1, default="M", choices=[
        ("M", "Mensuelle"),
        ("T", "Trimestrielle"),
        ("S", "Semestrielle"),
        ("A", "Annuelle"),
    ])
    proposition_montant_echeance_fcfa = models.DecimalField(max_digits=12, decimal_places=2, default=0)
    ...
    def calculer_capacite_endettement(self):
        """
        Calcule automatiquement la capacité d'endettement.
        
        Règle métier : La capacité d'endettement brute est de 40% du salaire net moyen.
        La capacité nette est la capacité brute moins les échéances en cours.
        
        Raises:
            ValueError: Si le salaire est négatif ou nul.
        """
        from decimal import Decimal
        from .constants import TAUX_ENDETTEMENT_MAX
        
        if self.salaire_net_moyen_fcfa <= 0:
            raise ValueError("Le salaire net moyen doit être positif")
        ...
        # Capacité nette = Capacité brute - échéances en cours
    ...
    def update_doc_flags_from_pieces(self):
        """Met à jour les drapeaux documents (étape 4) en fonction des pièces du dossier."""
