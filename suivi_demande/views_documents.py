"""
Vues pour la gestion des documents d'un dossier de crédit
"""
...
CATEGORIES_DOCUMENTS = {
    'JURIDIQUE': {
        ...
        'color': 'primary',
        'documents': [
            {'type': 'BILLET_ORDRE', 'label': 'Billet à ordre', 'obligatoire': True},
            {'type': 'DOMICILIATION', 'label': 'Attestation de domiciliation irrévocable', 'obligatoire': True},
        ]
    },
    'EMPLOYEUR': {
        ...
        'documents': [
            ...
            {'type': 'ATTESTATION_EMPLOYEUR', 'label': 'Attestation de l\'employeur', 'obligatoire': True},
        ]
    },
    'ASSURANCE': {
        ...
        'color': 'warning',
        'documents': [
            {'type': 'ASSURANCE_DECES', 'label': 'Assurance décès', 'obligatoire': True},
            {'type': 'ASSURANCE_INVALIDITE', 'label': 'Assurance invalidité', 'obligatoire': True},
        ]
    },
    'IDENTITE': {
...
def upload_documents(request, dossier_id):
    """Page d'upload des documents par catégorie."""
    ...
    for cat_key, cat_data in CATEGORIES_DOCUMENTS.items():
        ...
        cat_fournis = sum(1 for d in cat_data['documents'] if d['obligatoire'] and d['type'] in docs_par_type)
    ...
    pourcentage_completion = int((total_fournis / total_obligatoires * 100)) if total_obligatoires > 0 else 0
